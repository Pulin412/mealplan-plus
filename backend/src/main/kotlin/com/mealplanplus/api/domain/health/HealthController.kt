...
class HealthController(private val service: HealthMetricService) {
    ...
    fun create(@Valid @RequestBody dto: HealthMetricDto, auth: Authentication) = service.create(dto, auth.name)
    ...
    fun createCustomType(@Valid @RequestBody dto: CustomMetricTypeDto, auth: Authentication) =
