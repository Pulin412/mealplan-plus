...
import { listFollowing, type PublicProfileSummaryDto } from "@/lib/api/social";
...
function Inner() {
  ...
  useEffect(() => { listFollowing(handle).then(setList).catch(() => setList([])); }, [handle]);
  return (
    <div className="min-h-screen" style={{ background: C.bg }}>
      <div className="flex items-center gap-2 px-3 py-3">
        <button onClick={() => router.back()} style={{ fontSize: 22, color: C.ink }}>‹</button>
        <h1 className="text-[17px] font-bold" style={{ color: C.ink }}>Following</h1>
      </div>
      <div className="max-w-md mx-auto px-4">
        {list === null ? <div className="py-8 text-center" style={{ color: C.muted }}>Loading…</div>
          : list.length === 0 ? <div className="py-8 text-center" style={{ color: C.muted }}>Not following anyone yet</div>
