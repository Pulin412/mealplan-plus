...
# ── unit overrides for the ingredients.json foods: count-based -> PIECE + grams per piece ──
...
def _gpp(v):
    return _num(v) if v not in (None, '') else 'NULL'

# Unified list = the 101 processed ingredients (unit assigned here) + curated extras from
# data/extra_foods.json (which already carry category + unit + grams_per_piece).
unified = [dict(x, unit=('PIECE' if x['name'] in PIECE else 'GRAM'),
                grams_per_piece=PIECE.get(x['name'])) for x in out]
extra_path = 'data/extra_foods.json'
extras = json.load(open(extra_path)) if os.path.exists(extra_path) else []
seen = {u['name'].lower() for u in unified}
for e in extras:
    if e['name'].lower() in seen:
        continue
    unified.append(e); seen.add(e['name'].lower())
...
for _x in unified:
    ...
    _rows.append(f"  (gen_random_uuid(), '{_n}', '{_c}', {_num(_x['calories'])}, {_num(_x['protein'])}, "
                 f"{_num(_x['carbs'])}, {_num(_x['fat'])}, '{_x['unit']}', {_gpp(_x.get('grams_per_piece'))}, "
                 f"true, true, false, now(), now())")

_sql = (
 '-- V3: seed shared system foods (macros per 100g/ml, with category). ' + str(len(_rows)) + ' items.\n'
...
print('regenerated V3 with', len(_rows), 'rows total (', len(out), 'base +', len(unified) - len(out), 'extra )')
