...
object DatabaseModule {
    ...
    private val MIGRATION_37_38 = object : Migration(37, 38) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // Reset sync state so all local data is re-pushed to backend on next sync.
            // Backend upserts by serverId so no duplicates are created.
            db.execSQL("UPDATE meals SET syncedAt = NULL")
            db.execSQL("UPDATE diets SET syncedAt = NULL")
            db.execSQL("UPDATE health_metrics SET syncedAt = NULL")
            db.execSQL("UPDATE grocery_lists SET syncedAt = NULL")
            db.execSQL("UPDATE food_items SET syncedAt = NULL")
        }
    }
    ...
    private val MIGRATION_36_37 = object : Migration(36, 37) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            // Column was already TEXT NOT NULL — no SQL change needed.
        }
    }
