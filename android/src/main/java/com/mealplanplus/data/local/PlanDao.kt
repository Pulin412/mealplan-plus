...
interface PlanDao {
    ...
    suspend fun getPlanForDate(userId: Long, date: String): Plan?

    @Query("SELECT * FROM plans WHERE userId = :userId AND date BETWEEN :startDate AND :endDate ORDER BY date")
    fun getPlansInRange(userId: Long, startDate: String, endDate: String): Flow<List<Plan>>
    ...
    @Query("""
        ...
        ORDER BY p.date
    """)
    fun getPlansWithDietNames(userId: Long, startDate: String, endDate: String): Flow<List<PlanWithDietName>>
    ...
    """)
    suspend fun getDietForDate(userId: Long, date: String): Diet?
