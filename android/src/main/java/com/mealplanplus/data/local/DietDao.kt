...
@Dao
interface DietDao {
    /** User's own diets + system diets (userId IS NULL). */
    @Query("SELECT * FROM diets WHERE (userId = :userId OR userId IS NULL) ORDER BY name ASC")
    fun getDietsForUser(userId: Long): Flow<List<Diet>>
    ...
    @Query("SELECT * FROM diets WHERE isFavourite = 1 AND (userId = :userId OR userId IS NULL) ORDER BY name ASC")
    ...
    @Query("SELECT COUNT(*) FROM diets WHERE userId = :userId OR userId IS NULL")
    suspend fun getDietCountForUser(userId: Long): Int

    /**
     * All diets with full macro summary — user-scoped.
     */
    @Query("""
        SELECT
        ...
        WHERE d.userId = :userId OR d.userId IS NULL
        GROUP BY d.id
        ORDER BY d.name
    """)
    ...
    /**
     * All diets with full macro summary in a single query (unfiltered).
     */
    @Query("""
        SELECT
            d.id, d.name, d.description, d.createdAt,
